...
def add_all_numbers(window, listbox):
    numbers = listbox.get(0, tk.END)
    if len(numbers) < 2:
        messagebox.showerror("Error", "There must be numbers to add!", parent=window)
        return
...
def subtract_numbers(window, listbox):
    numbers = listbox.get(0, tk.END)
    if len(numbers) < 2:
        messagebox.showerror("Error", "There must be numbers to subtract!", parent=window)
        return
    total = int(numbers[0].split(". ")[1]) - sum(int(number.split(". ")[1]) for number in numbers[1:])
    clear_list(listbox)
    listbox.insert(tk.END, f"{counter}. {total}")
def multiply_all_numbers(window, listbox):
    numbers = listbox.get(0, tk.END)
    if len(numbers) < 2:
        messagebox.showerror("Error", "There must be numbers to multiply!", parent=window)
        return
    total = 1
    for number in numbers:
...
def divide_all_numbers(window, listbox):
    numbers = listbox.get(0, tk.END)
    if len(numbers) < 2:
        messagebox.showerror("Error", "There must be at least two numbers to divide!", parent=window)
        return
...
def square_all_numbers(window, listbox):
    numbers = listbox.get(0, tk.END)
    if len(numbers) < 1:
        messagebox.showerror("Error", "There must be at least one number to square!", parent=window)
        return
    clear_list(listbox)
    for number in numbers:
...
def about(window):
    ...
    update_label.pack()
    version_label = tk.Label(about_window, text="Version 0.60.667-2")
    version_label.pack()
...
about_window = None
def create_window():
    delete_button = tk.Button(window, text="Delete Selected Entry", command=lambda: delete_selected_entry(listbox))
    delete_button.pack()
    ...
    file_menu.add_command(label="Open", command=lambda: open_file(window, listbox))
    ...
    math_menu.add_command(label="Add", command=lambda: add_all_numbers(window, listbox))
    math_menu.add_command(label="Subtract", command=lambda: subtract_numbers(window, listbox))
    math_menu.add_command(label="Multiply", command=lambda: multiply_all_numbers(window, listbox))
    math_menu.add_command(label="Divide", command=lambda: divide_all_numbers(window, listbox))
    math_menu.add_command(label="Square", command=lambda: square_all_numbers(window, listbox))
    math_menu.add_command(label="Define Algebraic Letter", command=lambda: define_algebraic_letter(window))
    more_algebra_menu = tk.Menu(math_menu, tearoff=0)
    math_menu.add_cascade(label="More Algebra...", menu=more_algebra_menu)
    more_algebra_menu.add_command(label="Convert Algebra", command=lambda: convert_algebra(window, listbox))
    listbox = tk.Listbox(window)
    listbox.pack()
    entry = tk.Entry(window)
    entry.pack()
    button_add = tk.Button(window, text="Add number", command=lambda: add_number(window, listbox, entry))
    button_add.pack()
    button_clear = tk.Button(window, text="Clear List", command=lambda: clear_list(listbox))
    button_clear.pack()
create_window()
messagebox.showinfo("Saving Info", "Defining file extensions have changed again! Starting from Version 0.60, clicking on the 'Change File Extension' button will now change the file extension of the file you are currently working on! No more navigating to the 'File' menu!")
